...
function EstablishmentFeedback() {
  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      ...
      const [latestRes, statsRes] = await Promise.all([
        fetchLatestOwnerFeedbackSummary(estId).catch(() => null),
        fetchFeedbackStats(estId).catch(() => null),
      ]);
      ...
      console.error('[Establishment Feedback] load summary failed', err);
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      ...
      await generateOwnerFeedbackSummary(estId, {
      ...
      console.error('[Establishment Feedback] summary generation failed', err);
      ...
      <section className="lgu-feedback-page feedback-studio">
        <header className="lgu-feedback-hero">
          <div>
            ...
            <h1>Establishment feedback</h1>
            <p className="muted">
              Handle visitor comments quickly and maintain response quality.
            </p>
          </div>
          ...
          <>
            ...
            <section className="feedbackx-shell">
              ...
              {feedbackState.loading && !feedbackState.items.length ? (
                ...
                <div className="feedbackx-empty">No feedback found.</div>
              )}
            ...
            <div className="modal-content">
              <form className="modal-form" onSubmit={submitReply}>
                <div className="form-row">
                  <label className="form-label" htmlFor="est-reply-text">Message</label>
                  <textarea
                    id="est-reply-text"
                    rows={4}
                    required
            ...
            <header className="modal-header">
              <div>
                ...
                {threadModal.data?.createdAt ? (
                  <p className="muted">Submitted {new Date(threadModal.data.createdAt).toLocaleString()}</p>
                ) : null}
              </div>
