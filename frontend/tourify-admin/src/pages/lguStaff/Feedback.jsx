...
function LguStaffFeedback() {
    ...
    const distribution = feedbackState.distribution ?? {};
        ...
        () => [
            ...
            value: feedbackState.summary?.average
                ? `${feedbackState.summary.average.toFixed(2)} ★`
                : '—',
            helper: `${feedbackState.total} total reviews`,
            },
            {
            label: 'Most recent',
            value: feedbackState.items[0]?.rating
                ? `${feedbackState.items[0].rating.toFixed(1)} ★`
                : '—',
            helper: feedbackState.items[0]?.review_text
                ? `"${feedbackState.items[0].review_text.slice(0, 40)}${feedbackState.items[0].review_text.length > 40 ? '…' : ''}"`
                : 'No latest comment',
            },
        ],
        [feedbackState.summary, feedbackState.items, feedbackState.total],
        );
        ...
        try {
            ...
            await lguReplyToFeedback(feedbackId, { reply: draft });
            ...
            {error ? (
                ...
                <section className="lgu-summary-grid">
                    ...
                    {summaryState.parsedSummary ? (
                        ...
                        <p className="muted">
                        No summary yet. Generate one to brief the municipal tourism head.
                        </p>
                    )}
                    </article>
                </section>
                ...
                <section className="lgu-reviews-panel">
                    ...
                    {feedbackState.loading ? (
                        ...
                        <article key={item.feedback_id} className="lgu-table-row">
                            ...
                            <div className="lgu-review-reply">
                            <textarea
                                rows={3}
                                placeholder="Draft a reply for record keeping…"
                            ...
                            <button
                                ...
                                disabled={replySubmitting === item.feedback_id}
                            >
